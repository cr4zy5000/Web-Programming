...
function filterProperties(propNames, obj)
{
    ...
    */
    ...
    /*
        starts like ({}, B)
        if (B in obj) [in checks the properties of the object]
    */
}
...
/*
4.2 Fetch API
Using the fetch function, write a program that makes an HTTP request to obtain information about animated movies 
and saves the names of all the movies in a JSON file. To get the list of movies, 
make an HTTP request to: https://api.sampleapis.com/movies/animation
...
/*
4.3 Using crypto.randomBytes with Promises
Create a function randomBytesPromise(size) that wraps the Node.js crypto.randomBytes function 
in a Promise by explicitly using the Promise constructor.
The function should return a Promise that resolves with the generated bytes as a Buffer.
...
/*
4.4 General promisifyFunction
Create a general function promisifyFunction(fn, ...args) that converts any Node.js 
callback-style function (err, result) into a Promise by explicitly using the Promise constructor.
*/
